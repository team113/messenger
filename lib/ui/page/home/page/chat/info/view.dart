        ...
        () => ExpandablePanel(
          ...
          expanded: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('label_direct_chat_link_in_chat_description'.t()),
                const SizedBox(height: 10),
                _padding(
                  ReactiveTextField(
                ...
                Row(
                  children: [
                    Text(
                      '${'label_transition_count'.t()}: ${c.chat?.chat.value.directLink?.usageCount ?? 0}',
                    ),
                    Expanded(
                      child: Row(
        ...
        () => Column(
          children: [
            ...c.chat!.members.values.map(
              (u) => ListTile(
                title: Text(u.value.name?.val ?? u.value.num.val),
                leading: AvatarWidget.fromUser(u.value),
                trailing: IconButton(
                  key: const Key('DeleteChatMember'),
                  icon: Icon(
                      u.value.id == c.me ? Icons.exit_to_app : Icons.delete),
                  onPressed: c.membersOnRemoval.contains(u.value.id)
                      ? null
                      : () => c.removeChatMember(u.value.id),
                ),
                onTap: () => router.user(u.value.id, push: true),
              ),
            ),
            ListTile(
              key: const Key('AddMemberButton'),
              title: Text('btn_add_participant'.t()),
