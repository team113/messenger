...
class ChatController extends GetxController {
          ...
          if (text.isNotEmpty ||
            ...
            _chatService
                .sendChatMessage(
                  ...
                  repliesTo: onlyDonation
                      ? []
                      : send.replied.map((e) => e.value).toList(),
  ...
  Future<void> _fetchChat() async {
    try {
      ISentrySpan transaction = _ready.startChild('start');

      _ignorePositionChanges = true;

      status.value = RxStatus.loading();
      fetchStatus.value = 'Getting chat...';
      ...
      transaction.finish();
      transaction = _ready.startChild('get');
      ...
      if (chat == null) {
        ...
        transaction.finish();
        transaction = _ready.startChild('fetch');
        ...
        if (itemId == null) {
          ...
          fetchStatus.value = 'Fetching messages...';
          await chat!.around();
          fetchStatus.value = 'Fetching messages... done';
          ...
          fetchStatus.value = 'Fetching around...';
          await animateTo(itemId!);
          fetchStatus.value = 'Fetching around... done';
        }

        transaction.finish();
        transaction = _ready.startChild('around');
