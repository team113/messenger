                    ...
                    switch (ProfileTab.values[i]) {
                      ...
                      case ProfileTab.notifications:
                        return Block(
                          ...
                          children: [_NotificationsWidget(c)],
                        );

                      case ProfileTab.storage:
                        return Block(
                          title: 'label_storage'.l10n,
                          children: [_StorageWidget(c)],
                        );

                      case ProfileTab.language:
                        return Block(
                          title: 'label_language'.l10n,
                          children: [_LanguageWidget(c)],
                        );

                      case ProfileTab.blacklist:
                        return Block(
                          title: 'label_blocked_users'.l10n,
                          children: [_BlockedUsersWidget(c)],
                        );
...
class _EmailsWidget extends StatelessWidget {
  final MyProfileController c;
  const _EmailsWidget(
    this.c, {
    Key? key,
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      return Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: widgets.map((e) => _Dense(child: e)).toList(),
      );
    });
  }
}
...
/// Returns addable list of [MyUser.emails].
class _PhonesWidget extends StatelessWidget {
  final MyProfileController c;
  const _PhonesWidget(
    this.c, {
    Key? key,
...
class _BackgroundMessage extends StatelessWidget {
  final bool fromMe;

  final String text;
  const _BackgroundMessage({
    Key? key,
    this.fromMe = true,
    this.text = '123',
  }) : super(key: key);
...
class _BlockedUsersWidget extends StatelessWidget {
  final MyProfileController c;
  const _BlockedUsersWidget(
    this.c, {
    Key? key,
  }) : super(key: key);
